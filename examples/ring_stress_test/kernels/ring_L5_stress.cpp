/*
 * ring_L5_stress.so — L5 (CLOS1) ring stress dispatch kernel
 * Discovers L4 pod peers, dispatches ring_L4_stress.so to each.
 */
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
void linqu_orch_entry(LinquRuntime* rt, uint64_t*, int) {
    LinquPeerList peers = rt->ops->query_peers(rt, 4);

    rt->ops->scope_begin(rt);
    for (int i = 0; i < peers.count; i++) {
        rt->ops->submit_task(rt, peers.peers[i], "ring_L4_stress.so", nullptr, 0);
    }
    rt->ops->scope_end(rt);

    rt->ops->wait_all(rt);
    rt->ops->log_info(rt, "L5 Ring Stress: dispatched ring_L4_stress to %d pods", peers.count);
