/*
 * ring_L4_stress.so — L4 (POD) ring stress dispatch kernel
 * Discovers L3 host peers, dispatches ring_L3_stress.so to each.
 */
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
void linqu_orch_entry(LinquRuntime* rt, uint64_t*, int) {
    ...
    for (int i = 0; i < peers.count; i++) {
        rt->ops->submit_task(rt, peers.peers[i], "ring_L3_stress.so", nullptr, 0);
    }
    rt->ops->scope_end(rt);

    rt->ops->wait_all(rt);
    rt->ops->log_info(rt, "L4 Ring Stress: dispatched ring_L3_stress to %d hosts", peers.count);
