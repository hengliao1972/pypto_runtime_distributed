/*
 * topo_L5_clos1.so — L5 (CLOS1/Supernode) topology kernel
 * Discovers L4 pod peers, dispatches topo_L4_pod.so to each.
 */
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
void linqu_orch_entry(LinquRuntime* rt, uint64_t*, int) {
    ...
    for (int i = 0; i < peers.count; i++) {
        rt->ops->submit_task(rt, peers.peers[i], "topo_L4_pod.so", nullptr, 0);
    }
    rt->ops->scope_end(rt);

    rt->ops->wait_all(rt);
    rt->ops->log_info(rt, "L5 CLOS1: dispatched topo_L4_pod to %d pods", peers.count);
