/*
 * topo_L6_clos2.so — L6 (CLOS2/Cluster) topology kernel
 * Top-level: discovers L5 supernode peers, dispatches topo_L5_clos1.so to each.
 */
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
void linqu_orch_entry(LinquRuntime* rt, uint64_t*, int) {
    ...
    for (int i = 0; i < peers.count; i++) {
        rt->ops->submit_task(rt, peers.peers[i], "topo_L5_clos1.so", nullptr, 0);
    }
    rt->ops->scope_end(rt);

    rt->ops->wait_all(rt);
    rt->ops->log_info(rt, "L6 CLOS2: dispatched topo_L5_clos1 to %d supernodes", peers.count);
