/*
 * dag_L6_dispatch.so — L6 (CLOS2) DAG dispatch kernel
 * Top-level: discovers L5 supernode peers, dispatches dag_L5_dispatch.so to each.
 */
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
void linqu_orch_entry(LinquRuntime* rt, uint64_t*, int) {
    LinquPeerList peers = rt->ops->query_peers(rt, 5);

    rt->ops->scope_begin(rt);
    for (int i = 0; i < peers.count; i++) {
        rt->ops->submit_task(rt, peers.peers[i], "dag_L5_dispatch.so", nullptr, 0);
    }
    rt->ops->scope_end(rt);

    rt->ops->wait_all(rt);
    rt->ops->log_info(rt, "L6 DAG: dispatched dag_L5_dispatch to %d supernodes", peers.count);
