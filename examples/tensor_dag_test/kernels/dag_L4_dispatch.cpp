/*
 * dag_L4_dispatch.so — L4 (POD) DAG dispatch kernel
 * Discovers L3 host peers and dispatches dag_L3_compute.so to each.
 */
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
void linqu_orch_entry(LinquRuntime* rt, uint64_t*, int) {
    ...
    for (int i = 0; i < peers.count; i++) {
        rt->ops->submit_task(rt, peers.peers[i], "dag_L3_compute.so", nullptr, 0);
    }
    rt->ops->scope_end(rt);

    rt->ops->wait_all(rt);
    rt->ops->log_info(rt, "L4 DAG: dispatched dag_L3_compute to %d hosts", peers.count);
