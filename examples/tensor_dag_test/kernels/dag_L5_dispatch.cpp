/*
 * dag_L5_dispatch.so — L5 (CLOS1) DAG dispatch kernel
 * Discovers L4 pod peers, dispatches dag_L4_dispatch.so to each.
 */
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
void linqu_orch_entry(LinquRuntime* rt, uint64_t*, int) {
    ...
    for (int i = 0; i < peers.count; i++) {
        rt->ops->submit_task(rt, peers.peers[i], "dag_L4_dispatch.so", nullptr, 0);
    }
    rt->ops->scope_end(rt);

    rt->ops->wait_all(rt);
    rt->ops->log_info(rt, "L5 DAG: dispatched dag_L4_dispatch to %d pods", peers.count);
